...
function addAppTag2Url(url){
  let retUrl = url;
  if(window.appreferer && window.appreferer.type === 'app'){
    if(url.includes('projectId=')){
      retUrl = `${url}&appTag=${window.appreferer.appTag}`
    }
  }
  return retUrl
}

export default function request(url, data, method='POST', timeout=6000){
  let fullUrl = url;
  if(!url.startsWith('http')){
    fullUrl = `${API_URL}/${url}`
  }

  fullUrl = addAppTag2Url(fullUrl)
 ...
 */
export function requestGet(url, data, timeout=6000){
  let fullUrl = url;
  if(!url.startsWith('http')){
    fullUrl = `${API_URL}/${url}`
  }
